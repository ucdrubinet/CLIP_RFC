...
if __name__ == "__main__":
    ...
    CLIP_RFC_1.load_state_dict(torch.load("Pcam/src/model/checkpoints/CustomCLIP:{percent}_{alpha}_{seed}.pt".format(percent=percent, alpha=0.2, seed=1)))

    CLIP_RFC_2 = CustomCLIP(config_2, in_features=in_features).to(device)
    CLIP_RFC_2.load_state_dict(torch.load("Pcam/src/model/checkpoints/CustomCLIP:{percent}_{alpha}_{seed}.pt".format(percent=percent, alpha=0.4, seed=1)))

    CLIP_RFC_3 = CustomCLIP(config_3, in_features=in_features).to(device)
    CLIP_RFC_3.load_state_dict(torch.load("Pcam/src/model/checkpoints/CustomCLIP:{percent}_{alpha}_{seed}.pt".format(percent=percent, alpha=0.6, seed=1)))

    CLIP_RFC_4 = CustomCLIP(config_4, in_features=in_features).to(device)
    CLIP_RFC_4.load_state_dict(torch.load("Pcam/src/model/checkpoints/CustomCLIP:{percent}_{alpha}_{seed}.pt".format(percent=percent, alpha=0.8, seed=1)))

    CLIP_RFC_5 = CustomCLIP(config_5, in_features=in_features).to(device)
    CLIP_RFC_5.load_state_dict(torch.load("Pcam/src/model/checkpoints/CustomCLIP:{percent}_{alpha}_{seed}.pt".format(percent=percent, alpha=1.0, seed=1)))
        
    
    CLIP_RFC_GROUP = [CLIP_RFC_0, CLIP_RFC_1, CLIP_RFC_2, CLIP_RFC_3, CLIP_RFC_4, CLIP_RFC_5]
    tprs = []
    aucs = []
    ...
    for CLIP_RFC in CLIP_RFC_GROUP:
        ...
        aucs.append(roc_auc)
        if(alpha == 0):
            plt.plot(fpr, tpr, lw=2, alpha=0.3, label='Original CLIP (AUC = %0.2f)' % (roc_auc))
        else:
            plt.plot(fpr, tpr, lw=2, alpha=0.3, label='ROC Alpha=%0.1f (AUC = %0.2f)' % (alpha, roc_auc))
        alpha += 0.2
